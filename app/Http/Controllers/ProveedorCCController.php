...
class ProveedorCCController extends Controller
{
    ...
    public function cuentaCorrientePorVentas(Request $request)
    {
        ...
        $ventas = DB::table('ventas')
            ...
            ->where('cuotas.cuotable_type', 'App\Ventas')
            ->select('socios.nombre AS socio', 'ventas.id AS id_venta', 'proovedores.razon_social AS proovedor', 'productos.nombre AS producto', 'ventas.nro_cuotas', DB::raw('SUM(cuotas.importe) AS totalACobrar'));

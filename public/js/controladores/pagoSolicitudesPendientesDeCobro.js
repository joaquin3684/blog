...
app.controller('pagoSolicitudesPendientesDeCobro', function ($scope, $http, $compile, $sce, NgTableParams, $filter, ServicioABM) {
    ...
    $scope.pagar = function(){
        ...
        ServicioABM.create(data, url).then(function () {
            pull('pagoSolicitudesPendientesDeCobro/comercializadores', 'comercializadores', 'paramsComercializadores');
        });
    }

    $scope.PullSolicitudes = function (idComercializador) {
        ServicioABM.pull('pagoSolicitudesPendientesDeCobro/solicitudesComer/'+idComercializador).then(function (returnedData) {
            $scope.solicitudes = returnedData;
            $scope.sumaTotalACobrarPorComercializador= $scope.solicitudes.map(solicitud => solicitud.montoACobrar).reduce((total, monto) => total + monto)
            $scope.paramsSolicitudes = ServicioABM.createTable(returnedData)
        });
    }

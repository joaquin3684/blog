...
app.controller('ABM', function ($scope, $http, $compile, $sce, NgTableParams, $filter, ServicioABM, UserSrv, clonarHtmlService) {
    ...
    $scope.Alta = {
        http: function () {
            ...
            var self = this
            ServicioABM.create($.param(data), url).then(function successCallback() {
        ...
        actualizarDatos: function (data) {
            var dato = {}
            data.forEach(({ name: nombre, value: valor }) => {
                dato[nombre.toString()] = valor
            });
            
            $scope.datosabm.push(dato)
            $scope.paramsABMS.total($scope.datosabm.length);
            $scope.paramsABMS.reload()

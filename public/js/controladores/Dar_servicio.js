...
app.controller("Dar_servicio", function($scope, $http, $compile, $q, UserSrv) {
  ...
  $scope.getImporte = function() {
    $scope.importe = Number(
      ($scope.montoPorCuota * $scope.nro_cuotas).toFixed(2)
    );
  };
  ...
  $scope.calcularVencimiento = function() {
    $scope.vencimiento = moment(
      moment().format("YYYY") + "-" + moment().format("MM") + "-20"
    ).toDate();
    $scope.vencimiento = moment($scope.vencimiento)
      .add($scope.nro_cuotas + 1 || 0, "months")
      .toDate();
  };
  ...
  $scope.crearMovimiento = function() {
    try {
      ...
      var vencimiento = moment($scope.vencimiento, "DD/MM/YYYY").format(
        "YYYY-MM-DD"
      );
      ...
      $http({
        ...
        function successCallback(response) {
          ...
          UserSrv.MostrarMensaje(
            "OK",
            "Se ha otorgado el servicio correctamente.",
            "OK",
            "mensaje"
          );
          return response.data;
